...
- Bench I/O expander jumper inventory (stage-1 node, recorded at build
  time — issue #22). One MRCS IOX32 board carries two MCP23017 chips
  and therefore two I2C addresses; the cpNode-IOX 16-bit boards carry
  one chip each:
  - IOX32 board: `0x20` — no loopback jumpers; `0x21` — two jumpers
  - cpNode-IOX 16-bit boards: `0x22` — all 8 port A pins jumpered to
    port B; `0x23`, `0x24` — no loopback jumpers
