...
class SerialCMRITransport : public CMRITransport {
 ...
 private:
  ...
  // when this is set. Echo discard may finish the own-frame budget on
  // this tick, then must feed any further RX — a prompt R arrives only
  // after deassert (interop 2.3.15 / E10), not during ETX drain (#112).
  ...
  /// at most this many RX bytes are thrown away (the Host's own echo on
  /// a misconfigured 2-wire driver). Surplus RX is fed to the decoder.
  /// A legitimate R cannot start until the Node has ETX and only reaches
  /// the Host after TXEN deasserts (E10). Zero when idle or exhausted.
