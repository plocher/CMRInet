// display.h — SSD1306 OLED status panel for the XiaoSMININode
// example.
//
// Copied from the XiaoNode example panel (ported from the donor
// cpNode Xiao_I2C NodeDisplay): a pure renderer with no CMRI, IOX,
// WiFi, or OTA semantics of its own. Two screen groups:
//   - live view: bit grid + activity spinners + net status
//   - OTA views: progress bar / success / error (full-screen takeover)
// This sketch uses the live view only (no OTA; it never calls
// setNet(), so the status line stays blank). A screen-ownership
// latch keeps loop()'s show() from painting over an OTA result; an
// error screen holds ~5 s, then live view resumes.
...
// ESP32-only file: the panel needs the ESP32 display stack. The
// guard keeps a wrong-target build of this folder down to the
// sketch's own #error.
